    ...
    class SsrSite extends util.ComponentResource {
      ...
      constructor(name: string, opts: SsrSiteProps) {
        ...
        function uploadS3Assets() {
          ...
          addFolderContents(
            path.join(sitePath, ".open-next/assets"),
            "_assets"
          );
        ...
        function createServerFunction() {
          const serverRole = new aws.iam.Role(`${name}-server-function-role`, {
        ...
        function createImageFunction() {
          const serverRole = new aws.iam.Role(
            `${name}-image-optimization-role`,
            {
              assumeRolePolicy: aws.iam.assumeRolePolicyForPrincipal({
                Service: "lambda.amazonaws.com",
              }),
