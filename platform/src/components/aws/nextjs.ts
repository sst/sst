...
export interface NextjsArgs extends SsrSiteArgs, DevArgs {
  /**
...
if(request.headers["cloudfront-viewer-longitude"]) {
   ...
   */
  public get url() {
    return all([this.cdn?.domainUrl, this.cdn?.url, this.dev?.url]).apply(
      ([domainUrl, url, dev]) => domainUrl ?? url ?? dev ?? URL_UNAVAILABLE,
    );
  }
