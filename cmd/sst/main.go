		...
		},
		{
			...
			Description: cli.Description{
				...
				Long: strings.Join([]string{
					...
					"",
					"You can navigate to these processes through the sidebar.",
					"",
					"1. **SST**: A watcher for your `sst.config.ts` that deploy your app",
					"2. **Functions**: Your functions are run [Live](/docs/live/) and",
					"their invocations are logged",
					"3. Components that have `dev.autostart` enabled",
					"   - These include `Service` and frontends like `Nextjs`, `Remix`, `Astro`, `StaticSite`, etc.",
					"   - Their `dev.command` is run as a separate process",
					...
					"",
					"There are also some keyboard shortcuts you can use to interact with these",
					"processes.",
					"",
					...
					"```",
					"",
					"This will only deploy your app and run your functions. You'll need to start your",
					"frontend separately",
					"",
					"To do this run `sst dev` in a separate terminal session by passing the command.",
					"For example:",
					"",
					"```bash frame=\"none\"",
					"sst dev next dev",
			...
			Flags: []cli.Flag{
				{
					...
					Description: cli.Description{
						...
						Long:  "Defaults to using the multiplexer or `mosaic` mode. Use `mode=basic` to turn it off.",
					},
				},
			},
			Args: []cli.Argument{
