		...
		},
		{
			...
			Description: cli.Description{
				...
				Long: strings.Join([]string{
					...
					"",
					"1. Deploys your app",
					"2. For components with `dev.autostart`",
					"   - Skip the deploy",
					"   - Run their `dev.command` in a new multiplexer pane",
					"   - This includes `Service` and frontends like `Nextjs`, `Remix`, `Astro`, `StaticSite`, etc.",
					"2. Watch your `sst.config.ts` and re-deploys changes",
