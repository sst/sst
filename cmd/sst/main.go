		...
		},
		{
			...
			Description: cli.Description{
				...
				Long: strings.Join([]string{
					...
					"   - Runs their `dev.command` in a separate pane",
					...
					"`Service`, `StaticSite`, or the frontends like `Nextjs`, `Remix`, `Astro`, etc.",
					"That's because these have their own local environments that's started by the",
					"multiplexer.",
					"",
					":::note",
					"Components that typically have their own local environment are not deployed",
					"by `sst dev`. These include the frontends like `Nextjs` or `StaticSite`) and",
