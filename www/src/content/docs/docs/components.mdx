...
So this takes either a `RoleArgs` object or a function that takes `RoleArgs`. Where [args, opts, and name](https://www.pulumi.com/registry/packages/aws/api-docs/iam/role/#constructor-syntax) are the arguments for the role constructor passed to Pulumi.
