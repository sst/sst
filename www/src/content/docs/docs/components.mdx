...
:::tip
Set common props across all your components with `$transform`.
:::

For example, to set the runtime for all function components.
...
This sets the runtime for any `Function` component that'll be created after this call. But it cannot be overridden by the component.

:::note
The `$transform` props cannot be overridden by the component when it's created.
:::

Above, the `args` and `opts` are what you'd pass to the `Function` component. Recall the 
