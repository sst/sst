...
SST can these steps automatically for the supported providers through a concept of _adapters_. These create the above DNS records on a given provider.
...
1. Configure the Vercel provider in your app. This includes adding the provider package and configuring the **Vercel Access Token**. [Learn more](/docs/component/vercel/dns/#configure-provider).

   ```bash
   sst add @pulumiverse/vercel
   ```

2. Use the [`sst.vercel.dns`](/docs/component/vercel/dns/) adapter.
