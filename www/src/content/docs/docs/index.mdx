...
What makes SST different is that your _entire_ app is **defined in code** — in a single `sst.config.ts` file. This includes databases, buckets, queues, Stripe webhooks, or any one of the **150+ providers**.
...
Read more about our [monorepo setup](/docs/set-up-a-monorepo/).
...
1. Higher-level components
   ...
   SST's built-in components like [`Nextjs`](/docs/component/aws/nextjs/) or [`Email`](/docs/component/aws/email/) make it easy for developers to add features to their app. You can use these without having to figure out how to work with the underlying Terraform resources. Read more about [Components](/docs/components/).

2. Linking resources

   SST makes it easy to link your infrastructure to your application and access them at runtime in your code. Read more about [Linking](/docs/linking/).

3. Dev mode

   Finally, SST features a local developer environment that deploys your app through a watcher, runs your functions [_Live_](/docs/live/), and starts your frontend, all together. Read more about [`sst dev`](/docs/reference/cli/#dev).

---

**How does SST make money?**

While SST is open-source and free to use, we also have the [Console](/docs/console/) that can auto-deploy your apps and monitor for any issues. It's optional and includes a free tier but it's a SaaS service. It's used by a large numder of teams in our community, including ours.
