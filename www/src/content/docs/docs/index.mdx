...
- There are no _"stacks"_ and no stack resource limits.
- There are no npm packages for the CLI, just the CLI and the `sst.config.ts`.
- The `bind` command has been merged into the `dev` command. You don't need to run both of them.
...
:::note
Ion currently **only supports Node.js** applications and **macOS, Linux, and WSL**. Support for other runtimes and OSes are on the roadmap.
:::
...
Just like configuring the frontend, you can configure backend features of your application _in code_. You can features like cron jobs, buckets, queues, databases, and more.
...
The above snippets are called **Components**. They are a way of defining the features of your application in code. You can define any feature of your application.

These will create the necessary infrastructure in your account, in this case AWS, all without using the AWS Console. Currently **Ion supports AWS and Cloudflare**.

You can learn more about [components](/docs/components/).

---

### Configure

SST's components come with sensible defaults designed to get your started easily. But they can also be configured completely. For example, for the `sst.aws.Function` can be configured with all the common Lambda function props.
...
But with SST you can take it a step fruther and transform how the Function component creates its low level infrastructure. In this case, Function also creates an IAM Role. You can transform the IAM Role using the `transform` prop.
...
You can read more about [transforms](/docs/components#transforms).

---

### Extend

If you want to add a feature to your app that is not supported by SST's components yet, you can use Pulumi components directly in your SST app. This is because components in SST are built on Pulumi components.
...
Pulumi's [AWS Classic](https://www.pulumi.com/registry/packages/aws/) provider is automatically included in your SST app.
...
For example, let's say your app has a Next.js frontend and an S3 bucket for file uploads. You can `link` them together.
...
Now, you can use SST's [Node client](/docs/reference/client/) to access the S3 bucket in your Next.js app.
...
You can learn more about [resource linking](/docs/linking/).
...
Alternatively, you can use SST in a monorepo. This is useful if you have multiple frontends or you are working on a large project.
...
```bash
https://ion.sst.dev/install | bash
```
