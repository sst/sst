...
Learn more about how to [configure your AWS credentials](https://docs.sst.dev/advanced/iam-credentials#loading-from-a-file).

---

### Console

SST also comes with a [Console](/docs/console/). It shows you all your apps, the resources in them, lets you configure _git push to deploy_, and also send you alerts for when there are any issues in your apps.
...
The reason this works is because when SST deploys the above app, it'll convert it into a set of commands. It then call AWS with your credentials to create the underlying resources. So the above components create a list of low level resources in AWS.

:::note
You are not directly responsible for the low level resources that SST creates.
:::

If you login to your AWS Console you can see what gets created internally. While these might look a little intimidating, these are all managed by SST and you are not directly responsible for them.

SST will create, track, and removes all the low level resources created by your app.
...
There's a difference between the two snippets above. One is your **infrastructure code** and the other is your **runtime code**. One is run while creating your app, while the other runs when your user's use your app.
...
While the state is backed up, if for some reason you delete your state locally and in your provider, SST won't be able to manage the resources anymore. To SST this app doesn't exist anymore!

:::danger
Do not delete the bucket that stores your app's state.
:::

To fix this you'll have to manually re-import all those resources back into your app. Learn more about [how state works](/docs/providers/#state).
...
You can fix some of this by running [`sst refresh`](docs/reference/cli/#refresh) but in general you should avoid doing this.
...
Similar to app names, stage cannot be renamed. So if you wanted to rename a `development` stage to `dev`; you'll need to first remove `development` and then deploy `dev`.
...
This deploys your app to your _personal_ stage in _dev mode_. This deploys your app a little differently and is optimized for local development.
