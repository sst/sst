...
Start the dev mode for your Astro app and link it to SST.
...
:::note
You are starting `next dev` wrapped in `sst dev astro dev`.
:::
...
When our app loads, we'll generate a pre-signed URL for the file upload and add it to the form.
...
Head over to `http://localhost:4321` and try **uploading an image**, you should see it upload and then download the image.
...
![SST Astro app](../../../../assets/docs/start-astro.png)
...
![Astro site in SST Console](../../../../assets/docs/astro-site-in-the-sst-console.png)
