...
Let's add a Postgres database using [Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Welcome.html). The `sst.aws.Postgres` resource also configures an RDS Proxy behind the scenes making it ideal for serverless applications. Update your `sst.config.ts`. 
