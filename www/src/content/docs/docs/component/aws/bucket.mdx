...
The HTTP headers that origins can include in requests to the bucket URL.
...
The HTTP methods that are allowed when calling the bucket URL.
...
The origins that can access the bucket URL.
...
The HTTP headers you want to expose to an origin that calls the bucket URL.
