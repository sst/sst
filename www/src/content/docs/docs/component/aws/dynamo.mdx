...
- `"keys-only"`: Only the keys of the fields of the modified items. If you are worried about the costs, you can use the `keys-only` stream type since it stores the least amount of data.
