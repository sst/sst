...
The examples above are for the built-in SST components. However, you might be using other resources from the Pulumi/Terraform ecosystem. Or you might want to link a different set of outputs than what SST exposes.
...
You can also wrap any Pulumi Resource class to make it linkable with the `Linkable.wrap` static method.
