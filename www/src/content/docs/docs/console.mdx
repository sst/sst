...
3. **Configure an environment**
   ...
   :::note
   By default, the stage names are generated based on the branch or PR that's configured.
   :::

   By default, stages are based on the branch name or PR. Branch names are sanitized, while PRs use the `pr-<number>` pattern. This can be configured, we'll look at this below.
