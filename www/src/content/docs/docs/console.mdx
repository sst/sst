...
The Console shows you the complete [state of the resources](/docs/providers#state) in your app. You can view:
...
1. **Enable the GitHub integration**
   ...
   :::tip
   You can only associate your workspace with a single GitHub org. You can create multiple workspaces in the Console if you have multiple GitHub orgs.
   :::
...
For example, if you want git pushes to the `main` branch auto-deploy the `production` stage and use a _large_ build machine, you set something like this in youe `sst.config.ts`.
...
The Console needs to know which account it needs to autodeploy into. You configure this under **App's Settings** > **Autodeploy**. Each environment takes a stage name. This is the stage that is being deployed. It also takes the AWS account and any envrionment variables needed for the build process.

The stage names come from the git event. By default:

1. A git push to a branch triggers an auto-deploy on the stage with the same name. The branch names are sanitized to only letters/numbers and hyphens.

2. A git push to a PR, triggers a deploy in a stage with the name `pr-<number>`.
