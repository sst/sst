...
SST provides a cloud native local development environment that gives you instantaneous feedback on edits made in your function code. Changes are watched, built, and live reloaded **under 10 milliseconds**. And you can use **breakpoints to debug** your functions in your favorite IDE.
...
If you have resources like RDS instances deployed inside a VPC, and you are not [using the Data API](#) to talk to the DB, you have two options when working locally.

### Connecting to a VPC

By default your local Lambda function cannot connect to them. You need to:
...
Alternatively, you can run the database server locally (ie. MySQL or PostgreSQL). And in your function code, you can connect to local server if `IS_LOCAL` is set:
...
Check out the tutorial if you are [Debugging with WebStorm](https://serverless-stack.com/examples/how-to-debug-lambda-functions-with-webstorm.html)
...
Check out the tutorial if you are [Debugging with IntelliJ IDEA](https://serverless-stack.com/examples/how-to-debug-lambda-functions-with-intellij-idea.html)
