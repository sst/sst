...
import { createRequestHandler as createNodeRequestHandler, readableStreamToString } from "@remix-run/node";
import { URL } from "url";
...
function convertCfRequestToNode(event) {
  ...
  const url = new URL(request.uri + search, `https://${request.headers["host"][0].value}`);
