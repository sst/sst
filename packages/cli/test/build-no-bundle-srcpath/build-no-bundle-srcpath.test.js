 ...
 */
test("build-no-bundle-srcpath", async () => {
  ...
  const handlerHashFiles = fs.readdirSync(path.join(srcPathBuildPath, handlerHash));
  ...
  const handlerZipDotBuildFiles = fs.readdirSync(path.join(handlerZipDir, '.build', handlerHash));
  ...
  const srcHandlerZipDotBuildFiles = fs.readdirSync(path.join(srcHandlerZipDir, '.build', srcHandlerHash));
  ...
  const cf = fs.readFileSync(path.join(appBuildPath, 'cdk.out', 'prod-build-no-bundle-srcpath-sample.template.json'));
  ...
  expect(cfnLambdas[0].Properties.Handler).toEqual(`.build/${handlerHash}/lambda.handler`);
  expect(cfnLambdas[1].Properties.Handler).toEqual(`.build/${srcHandlerHash}/lambda.handler`);
});
